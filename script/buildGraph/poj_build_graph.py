...
parser.add_argument('--target_program', type=str, default='poj', help='target_program type(poj/spec)')
parser.add_argument('--output', type=str, default='/workspace/SCIS/dataset/POJ/poj_data/modelInput', help='output path')
parser.add_argument('--input', type=str, default='/workspace/SCIS/dataset/POJ/poj_data/binaryAllType', help='input path')
parser.add_argument('--threads',type=int,default=4,help='amount of threads')
...
parser.add_argument('--debug', type=bool, default=0, help='debug on')
...
def analyze_allgraph(b, outpath):
    print(time.strftime('%d %H:%M:%S',time.localtime(time.time()))+'[DEBUG] analyze start')
    ...
    cfg = b.analyses.CFGEmulated(resolve_indirect_jumps=False,context_sensitivity_level=2, keep_state=True,state_add_options=angr.sim_options.refs) 
    #print(time.strftime('%d %H:%M:%S',time.localtime(time.time()))+'[DEBUG] cfg build finish')
    ...
    ddg = b.analyses.DDG(cfg,start=cfg.functions['main'].addr)
    A=np.array(nx.adjacency_matrix(ddg.graph).todense())
    #print(time.strftime('%d %H:%M:%S',time.localtime(time.time()))+'[DEBUG] DDG build finish',len(A))
...
class Analyze_list (threading.Thread):
    def __init__(self, threadID, name, binary_list):
        threading.Thread.__init__(self)
        self.threadID = threadID
        self.name = name       
        self.binary_list = binary_list
        print('> Thread',name,'create:', len(binary_list),'binary')

    def run(self):
        for binary in self.binary_list:
            worker = Analyze_binary(self.threadID+threadMax,self.name+'-worker',binary)
            worker.start()
            worker.join()
            # print('> start to analyze binary:',binary['binarypath'])
            # proj = angr.Project(binary['binarypath'], load_options={'auto_load_libs':False},default_analysis_mode='symbolic')
            # #proj = angr.Project(binary['binarypath'], load_options={'auto_load_libs':False})
            # #analyze_allgraph(proj,binary['outputpath'])
            # analyze_dfg(proj,binary['filename'])
        print ("Finish thread:" + self.name)

class Analyze_binary (threading.Thread):
    def __init__(self, threadID, name, binary):
        threading.Thread.__init__(self)
        self.threadID = threadID
        self.name = name       
        self.binary = binary
        #print('> Thread',name,'create to analyze binary:', binary['binarypath'])

    def run(self):
        print('> start to analyze binary:',self.binary['binarypath'])
        proj = angr.Project(self.binary['binarypath'], load_options={'auto_load_libs':False},default_analysis_mode='symbolic')
        #proj = angr.Project(self.binary['binarypath'], load_options={'auto_load_libs':False})
        analyze_allgraph(proj,self.binary['outputpath'])
        #analyze_dfg(proj,self.binary['filename'])
        #print ("Finish thread:" + self.name)


def get_file_from_path(target_path):
    allfiles = []
...
if __name__ == "__main__":
    ...
    if target_program != "poj":
        print("Warning wrong dataset.")
        exit()
    
    #get all binaries and put in list
    compiler_list = ['gcc']  #['gcc','llvm']
    compile_tag_list = ['O3']  #['O2','O3']
    binary_list = []
    for compiler in compiler_list:
        for comp_t in compile_tag_list:
            binary_path = input_path + '/' + compiler + '/' + comp_t 
            for classId in range(1,poj_class_max+1):
                class_path = binary_path + '/' + str(classId)
                print('Collect file from ',class_path,end=": ")
                filelist = get_file_from_path(class_path)
                print(len(filelist),'collected')
                for binarypath in filelist:
                    filename = binarypath.split('/')[-1]
                    outputpath = output_path + '/' + compiler + '/' + comp_t +'/' + compiler+'_'+comp_t+'_'+str(classId)+'_'+filename
                    newarg = {'compiler':compiler,'comp_t':comp_t,'classId':classId,'filename':filename,'binarypath':binarypath,'outputpath':outputpath}
                    binary_list.append(newarg)
    
    print('Collect',len(binary_list),'binary for',compiler,comp_t)
    #print('[DEBUG]',binary_list[0])
    threads = []
    if debug_on:
        binary_list = binary_list[0:300]
        threadMax = 1

    for i in range(threadMax):
        binaryPartMax = int(len(binary_list)/threadMax)
        print('PART:',i,':',i*binaryPartMax,'-',(i+1)*binaryPartMax)
        binary_list_part = binary_list[i*binaryPartMax:(i+1)*binaryPartMax]
        if i == threadMax-1:
            binary_list_part = binary_list_part + binary_list[(threadMax*binaryPartMax):]

        th_name = "Thread-"+str(i)
        t = Analyze_list(i, th_name, binary_list_part)
        threads.append(t)
